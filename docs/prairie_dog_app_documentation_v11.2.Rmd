---
title: "Prairie Dog Decision Tool v11.2"
subtitle: "PDOG MAPR - Prairie Dog Management and Planning Resource"
author: "Research Team:
    Lead - Dr Ana Davidson (CNHP); 
    Model Development & validation - Dr Gabrielle Barrile (UW); 
    Initial app development - Max Sergeyev (CSU); 
    App rebuild & model validation - Dr Bort Edwards (CNHP)"
...
Modifies the binary poison treatment raster to represent sub-100% treatment
effectiveness. Only applies to prairie dog lethal control areas — plague mitigation
(insecticide) areas are unaffected.

**Arguments:**

- `treatment_raster`: raster with 1 = treated pixel, 0 = untreated pixel
- `efficacy_pct`: numeric 1–99, the percentage of the treatment area that is
  effectively treated
- `mode`: one of `"scatter"` or `"cluster"` — `"full"` and `"uniform"` are handled
  separately at the point of use

**Modes:**

- **scatter**: A random `(1 - efficacy_pct/100)` proportion of treated pixels are set
  to 0, distributed randomly across the treatment area. Stochastic — each run produces
  a different spatial outcome.
- **cluster**: The same proportion of pixels are set to 0, but spatially clustered
  using a seed-and-grow approach, better representing colony-level treatment failure.
  Also stochastic between runs.
- **uniform** (applied at model step, not here): Colonization and persistence
  probabilities within the treatment area are multiplied by `(1 - efficacy_pct/100)`
  rather than zeroed. At default model thresholds this mode may produce results similar
  to full efficacy, which may itself reflect the strong recolonization capacity of
  prairie dogs. Downward adjustment of the recolonization threshold may produce greater
  differences.

```{r treatment-efficacy}
apply_treatment_efficacy <- function(treatment_raster, efficacy_pct, mode) {
  # treatment_raster: raster with 1 = treated, 0 = untreated
  # efficacy_pct: numeric 1-99
...
- Prairie dog density (per acre, user-adjustable; default 42)
- Total expenditure on prairie dog control

```{r economic-calculator}
calculate_economic_scenario <- function(acres_poisoned, acres_pdogs_before,
                                       acres_pdogs_after, colony_under_treatment,
  ...
  #   forage_change_controlled, forage_change_controlled_per_acre,
  #   forage_change_total, forage_change_total_per_acre,
  #   percentage_saved, total_cost
}
```
...
The UI is built using `navbarPage` with three main tabs:

1. **Study Region Tab**: Map orienting users to the tool's geographic scope
2. **Overview Tab**: Documentation and instructions
3. **Predict Tab**: Interactive prediction interface

## UI Structure

```{r ui-structure, eval=FALSE}
...
1. **Data source indicator** (for multi-year iterations)
2. **Step 0**: Optional year selector
3. **Step 1**: Current year colony upload
4. **Step 2**: Prairie dog control areas upload
5. **Step 3**: Plague treatment areas upload
6. **Step 3b**: Draw management areas
7. **Step 3c**: Prairie dog treatment efficacy (optional)
8. **Step 4**: Prior year colony upload
9. **Step 5**: Study region selection
10. **Climate parameters** (temperature, precipitation)
11. **Prediction thresholds** (growth and plague)
12. **Run Models button**
13. **Download button**

A **dummy data download button** is available at the top of the sidebar, providing sample colony and management shapefiles for East Pawnee and Thunder Basin sites to allow users to explore the app before uploading their own data.
...
values <- reactiveValues(
  ...
  economic_results = list(
    A = list(forage_change_controlled = 0, forage_change_controlled_per_acre = 0,
             forage_change_total = 0, forage_change_total_per_acre = 0,
             percentage_saved = 0, total_cost = 0),
...
**Scenario A: Average Prairie Dog Consumption Values**
- Uses empirical consumption data
- 42 prairie dogs per acre density
- 335:1 prairie dogs per cow ratio
- 25 lbs forage per cow per day

**Scenario B: No Influence on Forage**
- Assumes prairie dog areas provide 100% forage value
- No change in forage from prairie dog presence

**Scenario C: 50% Forage Value**
- Prairie dog colonies yield 50% of normal forage
- User inputs baseline production per acre (PPA)

**Scenario D: 0% Forage Value**
- Prairie dog colonies are considered barren
- User inputs baseline production per acre (PPA)
...
2. **Management Input**
   ...
   - Optionally selects a treatment efficacy mode and percentage (Step 3c)
...
**Key Predictors**:
- `dfc`: Distance from current colony edge
- `hotr`: Heat load index
- `contagion`: Landscape contagion metric
- `ws`: Winter-spring precipitation
- `wy`: Whole year precipitation
...
- `mnn_sq`: MNN squared term
- `cohesion`: Landscape cohesion metric
- `dp`: Distance to plague
- `dp_sq`: Distance to plague squared
- `psize`: Patch/colony size
- `zwssf`: Change in winter-spring to spring-fall precipitation
- `zztmax`: Maximum summer temperature
- `zztmax_sq`: Temperature squared term
...
Files persist during session but are cleaned up when:
...
Default 100m resolution balances:
- Computational speed
- Spatial accuracy
- Memory usage
...
Uses fast nearest neighbor search (FNN package) to:
- Handle large climate matrices efficiently
- Avoid interpolation complexity
- Ensure consistent results

### Model Prediction

glmmTMB predictions can be slow for large areas. The application:
- Provides progress notifications
- Disables "Run Models" button during execution
- Updates button appearance to show status
...
- File uploads (presence and format)
- Shapefile completeness (all components present)
- CRS compatibility
...
When errors occur:
- Landscape metrics: Uses reasonable defaults
- Uncertainty calculations: Falls back to simple estimation
- Distance calculations: Uses large default values
...
1. **Configurable Treatment Efficacy**: Prairie dog lethal control can be modelled at 100% effectiveness (default) or at a user-specified sub-100% efficacy using one of three spatial representations (see Step 3c). Plague mitigation (insecticide) remains assumed fully effective.
...
1. **Multi-Year Persistence**: Configurable carryover of management effects
2. **Climate Projections**: Integration with future climate scenarios
3. **Weather Extremes**: Handling of drought, floods, heat waves
4. **Management-Climate Interactions**: How climate affects control effectiveness
5. **Cost-Benefit Optimization**: Automated management strategy suggestions
6. **Batch Processing**: Analyze multiple scenarios simultaneously
7. **Enhanced Uncertainty**: Bootstrap or Bayesian approaches
8. **Mobile Optimization**: Improved touch interface for tablets/phones
9. **Data Export**: Enhanced download options (GeoJSON, KML, etc.)
...
- Direct contact with research team
...
For the complete, runnable code, see the source file: `prairie_dog_app_glm_stablev11_1.R`

---

# Acknowledgments

This tool was developed through collaborative efforts of:
